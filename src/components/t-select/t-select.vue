...
export default {
    ...
    methods: {
        ...
        keyupEvent(value) {
            if (this.remote) {
                ...
                this.curList = this.inputLabel
                    ? this.allData.filter(
                          (item) =>
                              item[this.childLabel] &&
                              item[this.childLabel].indexOf(this.inputLabel) >
                                  -1
                      )
                    : this.allData;
            }
        ...
        hidePanel() {
            setTimeout(() => {
                const curData = this.allData.filter(
                    (item) => item[this.childLabel] === this.inputLabel
                );
                if (curData.length === 0) {
                    this.clearValue();
                    this.curList = this.allData;
        ...
        value: {
            handler(val) {
                ...
                if (this.selectData.length) {
                    const curData = this.selectData.filter(
                        (item) => item[this.childValue] === this.inputValue
                    );
                    if (curData.length) {
