    ...
    <div class="t-table" :class="className">
        ...
        >
            ...
            >
                ...
                <template
                    v-slot="{ row, rowIndex }"
                    v-if="item.render || (item.slot && !item.editRender)"
                >
                    <slot
                        :name="item.render || item.slot"
                        :row="row"
                        :index="rowIndex"
                    ></slot>
                </template>
                ...
                <template
                    ...
                    v-if="item.editRender && !item.editRender.name"
                >
                    <slot
                        :name="item.render || item.slot"
                        :row="row"
...
export default {
    ...
    methods: {
        ...
        setFixed(type) {
            let currentKey =
                this.currentTableTitle.key || this.currentTableTitle.field;
            let fixedCol = JSON.parse(
                localStorage.getItem('FIXED_' + this.id) || '{}'
            );
            ...
            switch (type) {
                ...
                case 0:
                    ...
                    this.finalColumns[
                        this.currentTableTitle.currentIndex
                    ].fixed = 'left';
        ...
        showTableColumns(sortColumns, fixedColumns) {
            ...
            if (fixedColumns) {
                ...
                oldColumns.forEach((item) => {
                    const fixed_index = fixedKeyArr.indexOf(
                        item.key || item.field
                    );
                    if (fixed_index > -1) {
            ...
            if (
                ...
                oldColumns.forEach((item) => {
                    const sort_index = sortColumns.indexOf(
                        item.key || item.field
                    );
                    if (sort_index > -1) {
        ...
        filterInput({ option, row }) {
            return (row[option.label] || '')
                .toLocaleLowerCase()
                .includes(option.data.toLocaleLowerCase());
        },
        // 分页器改变
        ...
        exportCsv(config, customExcludeColumns) {
            ...
            let keys = [];
            let title = [];
            let excludeColumns = customExcludeColumns
                ? customExcludeColumns
                : ['index'];
        ...
        customConfig() {
            ...
            return {
                ...
                checkMethod: ({ column }) => {
                    return (
                        !column.type &&
                        DISABLED_KEYS.indexOf(
                            column.property || column.field || column.key
                        ) === -1
                    );
                },
            };
        ...
        editFlag() {
            return (item) => {
                return (
                    !item.type &&
                    DISABLED_KEYS.indexOf(
                        item.property || item.field || item.key
                    ) === -1 &&
